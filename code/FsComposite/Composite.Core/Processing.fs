...
module Processing =
    ...
    let rec applyScnToElement scn acc element =
        match scn with
        | [] -> acc
        | f :: scn_tail -> match f element with
                           | Cons(Nil, Nil) -> applyScnToElement scn_tail acc element
                           | x -> applyScnToElement scn_tail (LazyList.append acc x) element
